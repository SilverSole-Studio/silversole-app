...
- **Access via `context` helpers** (`core/utils/useful_extension.dart`): `context.colorScheme` (ColorScheme), `context.textTheme` (TextTheme — every slot pre-sized/weighted; pick the slot instead of `copyWith`), `context.tokens` (`AppTokens`: `rewardGold`, `onReward`, `dataOrange`, `alert`, `success`, `brandCyan`, `brandGradient`). Scales: `AppSpacing.*`, `AppRadius.*` (+ ready-made `.cardR`/`.pillShape`/etc.).
- **Always reach theme data through these `context` getters** — `context.colorScheme` / `context.textTheme` / `context.tokens` referenced inline. Do **not** call `Theme.of(context).colorScheme` / `.textTheme` directly, and do **not** introduce local `final cs = Theme.of(context).colorScheme` / `final tt = Theme.of(context).textTheme` variables.
...
2. **Connection orchestration** — `bleForegroundControlProvider` (`lib/shared/providers/ble_foreground_controller.dart`) is `ref.watch`ed once in `App`. It is **Android-only** (early-returns otherwise). It listens to `settingsProvider.preferredDevice` and calls `BleConnectionService.attach()` once per device, passing a `BleSessionHandlers` bundle; its own backoff timer only arms when `attach()` itself fails (adapter off, permission revoked).

   `BleConnectionService` (`lib/core/ble/ble_connection_service.dart`) owns the single live session and is **connection-state driven, not polled**. `attach()` subscribes to `BluetoothDevice.connectionState` and connects with **`autoConnect: false`** (a direct connection); every `connected` event re-runs the handshake, and every `disconnected` event arms an internal backoff reconnect (1/2/4/8/16/30 s). Do **not** switch this to `autoConnect: true`: fbp passes it straight to Android's `connectGatt(context, autoConnect, …)`, and that background acceptlist connection routinely never completes for a device the Bluetooth stack has not connected to before — a freshly added sole then silently never connects. The handshake order is deliberate: `requestMtu` (kept here so MTU and setup live in one place) → **one** `discoverServices()` whose result backs every characteristic lookup → reads/writes (device id, base timestamp) → `onReady` → notify subscriptions **last**, because the live IMU characteristic streams at 50 Hz and would otherwise starve the remaining round-trips. Notify subscriptions are retired by `device.cancelWhenDisconnected()` rather than a manual cache, and `onValueReceived` (not `lastValueStream`) is used so a `read()` elsewhere does not replay a packet into the telemetry pipeline. Do not reintroduce per-operation `connect()`/`discoverServices()` calls or an `isConnected` polling timer — that combination produced a reconnect loop that re-wrote the base timestamp every ~42 s.
3. **Android foreground service** — `startBleService()`/`stopBleService()` in `ble_service_channel.dart` call a `MethodChannel` (`com.dongyutech.silversole/ble_service`) handled by `MainActivity.kt`, which runs `BleForegroundService.kt` (a sticky foreground service with an ongoing notification) so BLE survives backgrounding.
